...
function formatDate(date) {
  ...
  return formattedDate.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil(
    (daysDifference / (1000 * 60 * 60 * 24) + 1) / 7
  );
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  let potentialFriday = getNextFriday(date);
  while (potentialFriday.getDate() !== 13) {
    potentialFriday = getNextFriday(potentialFriday);
  }
  return potentialFriday;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDay <= endDate) {
    for (let i = 0; i < countWorkDays && currentDay <= endDate; i += 1) {
      workSchedule.push(format(currentDay));
